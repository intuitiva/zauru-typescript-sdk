...
function waitForReauthPopup(origin) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!w) {
            reject(new Error("El navegador bloqueó la ventana emergente. Permite ventanas para este sitio e intenta de nuevo."));
            return;
        }
        let settled = false;
 ...
 */
export function requestReauthInPopup() {
    if (typeof window === "undefined") {
        return Promise.reject(new Error("Reauth solo disponible en el navegador."));
    }
    if (reauthPromise) {
    ...
    toast.message("Sesión expirada", {
        description: "Completa el inicio de sesión en la ventana emergente para continuar.",
        duration: 12_000,
    });
