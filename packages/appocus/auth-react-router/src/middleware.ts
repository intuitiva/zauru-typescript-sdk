...
export const authMiddleware = async ({ request }: { request: Request }) => {
  ...
  if (!session.has("code")) {
    console.log(
      "[appocus/auth] User is not authenticated, redirecting to login.",
    );
 ...
 */
export const loginMiddleware = async ({ request }: { request: Request }) => {
  const cookie = request.headers.get("Cookie") ?? "";
  const session = await getSession(cookie);

  if (session.has("code")) {
    console.log(
      "[appocus/auth] User is already authenticated, redirecting to home.",
    );
    throw redirect("/");
  }
};
