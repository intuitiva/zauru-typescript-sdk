...
export const add4pinosPoDiscountsHistory = async (
  ...
  discounts: PoDiscountHistory["discounts"]
) => {
  return handlePossibleAxiosErrors(async () => {
    ...
    const purchaseOrderResponse = await getPurchaseOrder(
      session,
      purchaseOrderId
    );
    ...
    const purchaseOrderHistory = purchaseOrder.webapp_table_rowables?.find(
      (x) =>
        x.webapp_rows?.webapp_table_id ===
    ...
    if (purchaseOrderHistory) {
      ...
      await update4pinosPoDiscountHistory(
        headers,
        session,
        purchaseOrderHistory.webapp_rows?.id ?? -1,
        {
      ...
      console.log(
        "No existe una tabla de historial de porcentaje de rechazo - Creando nueva..."
      );
