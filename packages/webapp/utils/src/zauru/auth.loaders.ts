...
export type LoginWebAppData = NonNullable<
  Awaited<ReturnType<typeof loginWebApp>>["data"]
>;
...
const loginErrorPayload = (description: string, title?: string) => ({
  error: true,
  type: "error",
  title:
    title ??
...
const authorizeUrl = (request: Request): string => {
  ...
  const hostname = `${url.port ? "http://" : "https://"}${url.hostname}${
    url.port ? `:${url.port}` : ""
  }`;
...
export function createLoginLoader(): LoaderFunction {
  ...
  return async ({ request }) => {
    ...
    if (loginResponse.error || !loginResponse.data) {
      return Response.json(
        loginErrorPayload(loginResponse.userMsg?.toString() ?? ""),
      );
    }
 ...
 */
export function createSessionGuardLoader(
  extras?: AuthAccessExtras,
): LoaderFunction {
  return async ({ request }) => {
...
export function createLogoutAction(): ActionFunction {
  return async ({ request }) => {
    const session = await getSession(request.headers.get("Cookie"));
 ...
 */
export function createIndexLoader(extras?: AuthAccessExtras): LoaderFunction {
  return async ({ request }) => {
    const session = await getSession(request.headers.get("Cookie"));
    if (session.has("username")) {
      if (isDenied(extras?.requireAccess?.(session))) {
 ...
 */
export function createReloadCatalogsLoader(
  extras?: AuthAccessExtras,
): LoaderFunction {
  return async ({ request }) => {
    const session = await getSession(request.headers.get("Cookie"));
