...
import { get4pinosPoDiscountHistory, get4pinosSolicitudEliminacionPO, get4pinosWeightLimitPerBasket, getAgencies, getAllForms, getAuthorizationUpdateDiscountPO, getBookings, getCaseFormSubmissionsByCaseId, getCases, getCCPorcentajesDeRechazo, getClientCategories, getCurrencies, getEmployees, getFormsByDocumentType, getHeaders, getInvoiceFormSubmissionsByAgencyId, getInvoiceFormSubmissionsByInvoiceId, getInvoicesByAgencyId, getItems, getMotivosRechazo, getMyAgencyLotStocks, getMyCaseFormSubmissions, getPayeeCategories, getPayeeCategoriesByNotesMatch, getPayees, getPaymentMethods, getPaymentTerms, getPrintTemplates, getProviderCategories, getProviders, getSession, getSuggestedPrices, } from "@zauru-sdk/services";
...
import { getItemServicesByLabCategory, getItemsByLabCategory, getItemsByReceptionCategory, getLabItemCategories, } from "./items.utils.js";
import { getClientesLaboratorio, TEXT_PAYEE_CATEGORY_NOTES_FOR_PRICE, } from "./payees.utils.js";
...
const catalogErrorPayload = (catalog, description) => ({
    [catalog]: [],
    error: true,
    type: "error",
...
const sdkCatalogHandlers = {
    ...
    invoiceFormSubmissions: ({ session, url }) => {
        const agency_id = url.searchParams.get("agency_id") ?? session.get("agency_id");
    ...
    employeesByLab: ({ headers, session }) => getEmployeesByLabAgency(headers, session),
    employeesByCurrentAgency: ({ session }) => getEmployeesByCurrentAgency(session),
    ...
    invoicesByLab: ({ headers, session }) => getInvoicesByLabAgency(headers, session),
    ...
    payeeCategoriesLabPrices: ({ session }) => getPayeeCategoriesByNotesMatch(session, TEXT_PAYEE_CATEGORY_NOTES_FOR_PRICE),
    payeeCategories: ({ session }) => getPayeeCategories(session),
    receptionTypes: ({ headers, session }) => getReceptionTypes(headers, session),
    bitacoraRechazoMasivo: ({ headers, session }) => getBitacorasPOMassive(headers, session),
    cases: ({ session, url }) => {
        const tag_id = url.searchParams.get("tag_id") ?? undefined;
    ...
    myCases: ({ session }) => getCases(session, {
        responsible_id: Number(session.get("employee_id")),
    }),
    myCaseFormSubmissions: ({ headers, session }) => getMyCaseFormSubmissions(headers, session),
    ...
    ccPorcentajesDeRechazo: ({ headers, session }) => getCCPorcentajesDeRechazo(headers, session),
    programaciones4pinos: ({ headers, session }) => getProgramaciones(headers, session),
    solicitudesEliminacionPO: ({ headers, session }) => get4pinosSolicitudEliminacionPO(headers, session),
    pesoMaximoPorCanasta: ({ headers, session }) => get4pinosWeightLimitPerBasket(headers, session),
    poDiscountHistory: ({ headers, session }) => get4pinosPoDiscountHistory(headers, session),
    authorizationUpdateDiscountPO: ({ headers, session }) => getAuthorizationUpdateDiscountPO(headers, session),
    itemsByReception: ({ headers, session }) => getItemsByReceptionCategory(headers, session),
    itemsByLab: ({ headers, session }) => getItemsByLabCategory(headers, session),
    itemServicesByLab: ({ headers, session }) => getItemServicesByLabCategory(headers, session),
    itemCategoriesForLab: ({ headers, session }) => getLabItemCategories(headers, session),
    bundlesRecipForLab: ({ headers, session }) => getBundlesRecipByLabCategory(headers, session),
    bundlesForLab: ({ headers, session }) => getBundlesByLabCategory(headers, session),
    ...
    suggestedPrices: ({ session, url }) => {
        ...
        const withItemCategories = url.searchParams.get("withItemCategories") === "true";
    ...
    invoiceFormSubmissionsByInvoiceId: async ({ headers, session, url }) => {
        ...
        return getInvoiceFormSubmissionsByInvoiceId(headers, session, invoiceId, withFiles);
    },
 ...
 */
export function createCatalogsLoader(extras) {
    return async ({ request }) => {
        try {
            ...
            if (result.error || !result.data) {
                return Response.json(catalogErrorPayload(catalog, `${result.userMsg ?? ""}`));
            }
            return { [catalog]: result.data };
        }
        catch (error) {
            console.error(error);
            const catalog = new URL(request.url).searchParams.get("catalog") || "catalog";
            return catalogErrorPayload(catalog, `${error}`);
        }
    };
}
export function CatalogsRoute() {
    return createElement("div", { style: { fontFamily: "system-ui, sans-serif", lineHeight: "1.4" } }, "CATALOGS LOAD");
}
