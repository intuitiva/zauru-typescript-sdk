...
export const getDepSelectOptions = (): SelectFieldOption[] => {
  const optionsDep =
    guatemalaCountryInfo?.map((x) => {
      const value = x.label.split(",")[0];
      return { label: value, value } as SelectFieldOption;
    }) ?? [];
...
export const getMunSelectOptions = (): SelectFieldOption[] => {
  return (
    guatemalaCountryInfo?.map((x) => {
