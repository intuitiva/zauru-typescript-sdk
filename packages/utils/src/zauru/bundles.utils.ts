 ...
 * @returns
 */
export const getBundlesRecipByLabCategory = async (
  headers: any,
  session: Session
): Promise<AxiosUtilsResponse<BundleGraphQL[]>> => {
  return handlePossibleAxiosErrors(async () => {
    const { lab_bundles_recip_item_category_id } = await getVariablesByName(
      headers,
      session,
      ["lab_bundles_recip_item_category_id"]
    );

    const response = await getBundlesByItemCategoryId(
      session,
      lab_bundles_recip_item_category_id
    );
    ...
    if (response.error) {
      throw new Error(
        `Ocurrió un error al consultar los paquetes por categoría de laboratorio: ${response.userMsg}`
      );
    }
 ...
 * @returns
 */
export const getBundlesByLabCategory = async (
  headers: any,
  session: Session
): Promise<AxiosUtilsResponse<BundleGraphQL[]>> => {
  return handlePossibleAxiosErrors(async () => {
    const { lab_bundles_item_category_id } = await getVariablesByName(
      headers,
      session,
      ["lab_bundles_item_category_id"]
    );
 ...
 * @returns
 */
export const createNewLabPaquete = async (
  headers: any,
  session: Session,
  ...
  return handlePossibleAxiosErrors(async () => {
    ...
    const sendBody: Partial<BundleGraphQL> = {
      ...body,
 ...
 * @returns
 */
export const createNewLabEnsayo = async (
  headers: any,
  session: Session,
  body: Partial<BundleGraphQL>
): Promise<AxiosUtilsResponse<boolean>> => {
  return handlePossibleAxiosErrors(async () => {
    ...
    //PASO 2
    //Creamos el item
    const itemSendBody: Partial<ItemGraphQL> = {
    ...
    //PASO 3
    //Creo el formulario
    const formSendBody: Partial<FormGraphQL> = {
      active: true,
 ...
 * @returns
 */
export const deleteLabBundle = async (
  headers: any,
  session: Session,
  bundle: BundleGraphQL
): Promise<AxiosUtilsResponse<boolean>> => {
  return handlePossibleAxiosErrors(async () => {
    await deleteBundle(headers, bundle.id);

    //busco el item-servicio para eliminarlo
    const responseItem = await getItemByName(
      session,
      `LAB_SERVICE_${bundle.id}; ${bundle.name}`
    );
    ...
    const responseForm = await getFormByName(
      session,
      `LAB_FORM_${bundle.id}; ${bundle.name}`
    );
