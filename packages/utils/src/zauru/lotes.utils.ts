...
import { createNewReception, enablePurchase, getDeliveryByBooking, getLote, getLotesFiltered, getPurchase, getPurchaseOrder, getVariablesByName, insertBookings, saveMotivosDeRechazoByPurchase, updateLote, updatePurchaseOrder } from "@zauru-sdk/services";
import { BasketSchema, InsertBookingBody, NewReceptionBody, UpdatePurchaseOrderBody } from "@zauru-sdk/types";
...
export const procesarLote = async (
  ...
  const baskets_regresadas = basket_keys.map((basket_key: string) => {
  ...
  if (purchaseOrderResponse.error || !purchaseOrderResponse?.data) {
    throw new Error(
      `Error al intentar encontrar la órden de compra: ${purchaseOrderResponse.userMsg}`
    );
  }
  ...
  if (!byPercent) {
    ...
    const bodyGoodToProduction: InsertBookingBody = {
      booker_id: Number(session.get("employee_id")),
      needs_transport: false,
      payee_id: purchaseOrder.payee_id,
      memo: `${purchaseOrder.lots[0].id};${purchaseOrder.id_number};${
        purchaseOrder.purchase_order_details[0]?.item?.name
      };${purchaseOrder.purchase_order_details
        .map((purchase) => Number(purchase.booked_quantity))
        .reduce(reduceAdd, 0)};${baskets_memo_quantity};${porcentajeRechazo};${
     ...
     */
    baskets_regresadas.forEach((basket: BasketSchema) => {
    ...
    const goodVegetablesInserted = await insertBookings(
      headers,
      bodyGoodToProduction,
      `${purchaseOrder.id}`
    );
    ...
    if (lbsMalas && lbsMalas > 0) {
      ...
      const bodyOchToProduction: InsertBookingBody = {
        booker_id: Number(session.get("employee_id")),
        needs_transport: false,
        memo: "",
        planned_delivery: `${data.date}`,
      ...
      console.log(
        "--------- enviando a insertar booking de rechazo a la bodega de producción"
      );
      const ochToProductionVegetablesInserted = await insertBookings(
        headers,
        bodyOchToProduction,
        `${purchaseOrder.id}`
      );
      ...
      if (
        !ochToProductionVegetablesInserted.error &&
        ...
        await getDeliveryByBooking(
          headers,
          ochToProductionVegetablesInserted.data.id
        );
      }
      //=====================================================================================
      //=====================================================================================
      ...
      console.log(
        "--------- enviando a insertar booking de rechazo a la bodega de rechazo"
      );
      const ochToOchVegetablesInserted = await insertBookings(
        headers,
        bodyOchToProduction
      );

      if (
        !ochToOchVegetablesInserted.error &&
        ...
        await getDeliveryByBooking(
          headers,
          ochToOchVegetablesInserted.data?.id
        );
      }
    }
    ...
    const newDescriptions: Array<string> = [];
    baskets_memo.forEach((basket: BasketSchema) => {
      //Si existe una basket a regresar con el mismo id, pregunto cuantas va a regresar, para restarlas del total.
      const basket_regresada = baskets_regresadas.find(
        (x) => x.id === basket.id
      );
      const resta = basket_regresada?.total ?? 0;
      newDescriptions.push(
        `${basket.total - resta}-${basket.id}-${basket.color}`
      );
    });
    ...
    const bodyGoodToProduction: InsertBookingBody = {
      ...
      needs_transport: false,
      payee_id: purchaseOrder.payee_id,
      memo: `${purchaseOrder.lots[0].id};${purchaseOrder.lots[0].name};${
        purchaseOrder.purchase_order_details[0]?.item?.name
      };${purchaseOrder.purchase_order_details
  ...
  await saveMotivosDeRechazoByPurchase(
    headers,
    session,
    {
