...
import {
  deleteQueueFormReceptionHistory,
...
export const register4pinosReception = async ({
  ...
  let apiResponses: ApiResponseFor4pinosReceptions = {
  ...
  try {
    ...
    if (apiResponses.apiCall === 2) {
      ...
      const responsePOSearch = await getPurchasesOrderByIdNumber(
        session,
        values?.idNumberInput
      );

      if (responsePOSearch.error || !responsePOSearch.data) {
        throw new Error(
          `No hay comunicación con la red para la validación de exists - Validate Exists: ${responsePOSearch?.userMsg}`
        );
      }
    ...
    const {
      ...
      recepciones_vendor_agency_id: origin_agency,
    ...
    const provenance = values.selectedProvenance as string;
    const purchaseOrderGeneralInfo = JSON.parse(
      values?.purchaseOrderGeneralInfo?.toString() ?? "{}"
    ) as PurchaseOrderGeneralInfo;
    let controlCalidadValues: {
      porcentajeRechazo: string;
      razonPrincipal: string;
      razonSecundaria: string;
      razonTerciaria: string;
      otrasRazones: string;
    } = {
      porcentajeRechazo: values.porcentajeRechazo as string,
      razonPrincipal: values.razonPrincipal as string,
      razonSecundaria: values.razonSecundaria as string,
      razonTerciaria: values.razonTerciaria as string,
      otrasRazones: values.otrasRazones as string,
    };
    ...
    const buildSimpleArray = (keyIncludes: string, processValue: any) => {
      const regex = /\d/; // Expresión regular para buscar al menos un dígito.
      return keys
        .filter((key) => key.includes(keyIncludes) && regex.test(key))
        .map((key) =>
    ...
    if (apiResponses.apiCall === 2) {
      ...
      const newAuthorizedPurchaseResponse =
        await createNewAuthorizedPurchaseOrder(
          headers,
      ...
      if (
        newAuthorizedPurchaseResponse?.error ||
    ...
    if (apiResponses.apiCall === 3) {
      ...
      const responseNewReception = await createNewReception(
        headers,
        newReception as any,
        apiResponses.authorizedPO.id
      );

      if (responseNewReception.error || !responseNewReception.data) {
        throw new Error(
          "Error al intentar crear la recepción: " +
    ...
    if (apiResponses.apiCall === 5) {
      ...
      const getShipmentResponse = await getDeliveryByBooking(
        headers,
        apiResponses.shipment.id
      );

      if (getShipmentResponse.error) {
        throw new Error(
          "Error al intentar recibir el envío: " + getShipmentResponse?.userMsg
        );
      }
      apiResponses.apiCall = apiResponses.apiCall + 1;
    }
    ...
    if (!esCentroDeAcopio) {
      ...
      if (apiResponses.apiCall === 7) {
        ...
        const getQCDeliveryResponse = await getDeliveryByBooking(
          headers,
          apiResponses.qcShipment.id
        );
      ...
      if (apiResponses.apiCall === 8 || qualityControlBaskets.length === 0) {
        if (
          ...
          console.log(
            "----------- Guardando rechazo de canastas directo...... -----------"
          );
          ...
          if (responseRechazo.error) {
            throw new Error(
              "Error al intentar guardar el rechazo de canastas directo: " +
      ...
      if (
        ...
        if (apiResponses.apiCall === 6) {
          ...
          if (saveMotivosResponse.error) {
            throw new Error(
              "Error al intentar guardar los motivos de rechazo: " +
