 ...
 * @returns
 */
export const getShipmentsToMyAgency = async (
  session: Session
  ...
  return handlePossibleAxiosErrors(async () => {
    ...
    if (response.error) {
      throw new Error(
        `Ocurrió un error al consultar los envíos a mi agencia actual: ${response.userMsg}`
      );
    }
 ...
 * @returns
 */
export const shipmentToVirtualClientAgency = async (
  headers: any,
  session: Session,
  props: {
  ...
  return handlePossibleAxiosErrors(async () => {
    const { virtual_client_agency_id } = await getVariablesByName(
      headers,
      session,
      ["virtual_client_agency_id"]
    );

    let bookingBody: Partial<InsertBookingBody> = {};

    if (props?.esPerecedero) {
      const responseStocks = await getMyAgencyLotStocks(session);
      if (!responseStocks.data || responseStocks.error) {
        throw new Error(
          `Ocurrió un error al tratar de obtener los stocks de mi agencia. ${responseStocks.userMsg}`
        );
      }
      ...
      if (
        itemStocks.map((x) => x.available).reduce(reduceAdd, 0) <
        props.deliver_quantity
      ) {
        throw new Error(
          `No hay suficiente stock en los lotes para realizar el descuento.`
        );
      }

      const movements: Partial<MovementGraphQL>[] = itemStocks.reduce(
        (acc: Partial<MovementGraphQL>[], stock) => {
    ...
    if (response1.error || !response1.data) {
      throw new Error(
        `Ocurrió un error al tratar de crear el envío a agencia virtual de cliente: ${response1.userMsg}`
      );
    }
    ...
    if (response2.error || !response2.data) {
      throw new Error(
        `Ocurrió un error al tratar de recibir: ${response1.userMsg}`
      );
    }
