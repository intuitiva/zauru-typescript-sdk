...
async function getGraphQLToken(session) {
    return handlePossibleAxiosErrors(async () => {
        ...
        if (!token || tokenHasExpired) {
            tokenHasExpired
                ? console.log(`${CONSOLE_LOG_COLORS.FgYellow}%s${CONSOLE_LOG_COLORS.Reset}`, `=============== ⚠️ EL TOKEN GRAPHQL ESTÁ EXPIRADO ⚠️ ====================`)
                : console.log(`${CONSOLE_LOG_COLORS.FgYellow}%s${CONSOLE_LOG_COLORS.Reset}`, `=============== ⚠️ NO HAY UN TOKEN GRAPHQL GUARDADO ⚠️ ====================`);
            ...
            if (responseToken.data) {
                ...
                console.log(`${CONSOLE_LOG_COLORS.FgGreen}%s${CONSOLE_LOG_COLORS.Reset}`, `=============== ✅ TOKEN GRAPHQL GUARDADO EN SESION Y DEVUELTO ✅ ====================`);
                return responseToken.data;
            }
            console.log(`${CONSOLE_LOG_COLORS.FgRed}%s${CONSOLE_LOG_COLORS.Reset}`, `=============== ❗ NO HAY INFORMACIÓN OBTENIDA DEL REQUEST A ZAURU - GET_TOKEN ❗ ====================`);
...
export const getGraphQLAPIHeaders = async (session) => {
    ...
    if (error) {
        console.log(`${CONSOLE_LOG_COLORS.FgRed}%s${CONSOLE_LOG_COLORS.Reset}`, `=============== ❗ OCURRIÓ UN ERROR DEL REQUEST A ZAURU - GET_TOKEN ❗ ==================== ${userMsg}`);
        return {
