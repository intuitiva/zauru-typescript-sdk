 ...
 * @param specialItems
 */
export const guardarConfiguracionProductosEspeciales = (headers, session, configuration, specialItems) => {
    return handlePossibleAxiosErrors(async () => {
        specialItems?.forEach(async (x) => {
            if (!configuration.some((y) => y.id === x.id)) {
                //Elimino los que ya no están
                const response = await updateSpecialItem(headers, session, x.id.toString(), {
                    ...x.data,
    ...
    return handlePossibleAxiosErrors(async () => {
        ...
        for (let i = 0; i < costosWithPurchaseIds.length; i++) {
            ...
            console.log(`==============================> ACTUALIZANDO ÓRDEN DE COMPRA: ID: ${x.id} PRICE: ${x.purchase_order_details
                ...
                .map((x) => x.item_id)
                .join(",")}`);
            const updateResponse = await updatePurchaseItemPrice(headers, {
...
export const changePrices = (headers, session, bitacora, id_bitacora) => {
    return handlePossibleAxiosErrors(async () => {
        ...
        console.log(`==============================> GUARDANDO BITÁCORA DE CAMBIOS`);
        const response = await updateBitacoraCostos(headers, session, {
            ...bitacora,
            costosActualizados: costosWithPurchaseIds,
            costosEspecialesActualizados: costosEspecialesWithPurchaseIds,
        }, id_bitacora);
...
export const getCostosBitacora = (headers, session) => {
    return handlePossibleAxiosErrors(async () => {
        ...
        const response = await getWebAppTableRegisters(session, bitacora_costos_verduras_web_app_table_id);
        if (response.error) {
