...
export async function createNewLaboratoryClientCategoryPrice(headers, session, body) {
    return handlePossibleAxiosErrors(async () => {
        ...
        const createPriceResponse = await createPriceList(headers, sendBodyPriceList);
        ...
        const { lab_payment_term_default_id } = await getVariablesByName(headers, session, ["lab_payment_term_default_id"]);
        const paymentTermResponse = await getPaymentTermById(session, lab_payment_term_default_id);
