...
exports.shipmentToVirtualClientAgency = exports.getShipmentsToMyAgency = void 0;
 ...
 * @returns
 */
const getShipmentsToMyAgency = async (session) => {
    return (0, common_1.handlePossibleAxiosErrors)(async () => {
        const response = await (0, services_1.getShipmentsByToAgencyLast100Id_booking)(session, session.get("agency_id"));
        if (response.error) {
...
exports.getShipmentsToMyAgency = getShipmentsToMyAgency;
/**
 *
 * @param headers
 * @param session
 ...
 * @returns
 */
const shipmentToVirtualClientAgency = async (headers, session, props) => {
    return (0, common_1.handlePossibleAxiosErrors)(async () => {
        const { virtual_client_agency_id } = await (0, services_1.getVariablesByName)(headers, session, ["virtual_client_agency_id"]);
        let bookingBody = {};
        if (props?.esPerecedero) {
            const responseStocks = await (0, services_1.getMyAgencyLotStocks)(session);
            ...
            if (itemStocks.map((x) => x.available).reduce(common_1.reduceAdd, 0) <
            ...
            bookingBody = {
                ...
                planned_delivery: (0, common_1.getDatePickerCurrentDate)(),
        ...
        const response1 = await (0, services_1.insertBookings)(headers, bookingBody);
        ...
        //Entregar el envío
        const response2 = await (0, services_1.getDeliveryByBooking)(headers, response1.data.id);
...
exports.shipmentToVirtualClientAgency = shipmentToVirtualClientAgency;
